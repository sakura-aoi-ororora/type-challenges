...
type IsEqual<X, Y> = (<T>() => T extends X ? string : number) extends <T>() => T extends Y
    ? string
    : number
    ? true
    : false;

type Includes<T extends readonly any[], U> = T extends [infer A, ...infer B] ? IsEqual<A, U> extends true ? true : Includes<B, U> : false
